...
closeButton.addEventListener('click',()=>{
  const sidebar = document.getElementsByClassName('sidebar-chat-history')[0].style.display = 'none'
  document.getElementsByClassName('sidebar-backdrop')[0].style.display = 'none'
})
  }
...
<svg onClick={closeSidebarHandler} fill="skyblue" className="sidebar-close" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="33" height="33" viewBox="0 0 50 50">
<path d="M 9.15625 6.3125 L 6.3125 9.15625 L 22.15625 25 L 6.21875 40.96875 L 9.03125 43.78125 L 25 27.84375 L 40.9375 43.78125 L 43.78125 40.9375 L 27.84375 25 L 43.6875 9.15625 L 40.84375 6.3125 L 25 22.15625 Z"></path>
</svg>
   <div className="sidebar-new-chat">
  <div title="Click here to open new chat window" onClick={()=>{
    router.push("/chat")
  }}>
   <Link style={{textDecoration : 'none', color : 'aliceblue', fontSize : '1.2rem', paddingLeft :'3rem', fontWeight : 'bold'}} href="/chat">New chat</Link>
   <FaEdit cursor={"pointer"} size={20} color="aliceblue"/>
   </div>
   </div>
   <div title="Your old history" className="sidebar-old-chats-history">
    
    <svg xmlns="http://www.w3.org/2000/svg" height="32" width="32" viewBox="0 0 32 32" id="history" ><path fill="white" d="M30,16A13,13,0,0,1,6.54,23.72l1.61-1.19a11,11,0,1,0-1.67-9.71l1-.65,1.12,1.66-3,2A1,1,0,0,1,5,16a1,1,0,0,1-.83-.44l-2-3,1.66-1.12.68,1A13,13,0,0,1,30,16ZM16,9v7a1,1,0,0,0,.29.71l3,3,1.42-1.42L18,15.59V9Z" data-name="03  History, Recent"></path></svg> 
      </div>
  <div className="sidebar-old-chats">
    
    {historyList.length<=0 && 
    <div title="No history found!" className="sidebar-nothing-here"><img className="sidebar-nothing-here-image" src="/Nothing.webp"></img></div>}
    {historyList.map((history)=>{
      return(
        <div  key={history._id} onClick={()=>{
          router.push(`/chat/${history._id}`)
        }}  className={`sidebar-old-chats-content ${history._id === id?'sidebar-active-history-link' : ""}`}>
          <div title="Your chat">
        ...
        <span className="sidebar-question-span-title"> <Link style={{textDecoration : 'none', color : "aliceblue"}} key={history._id} href={`/chat/${history._id}`}>{history.title}</Link></span>
        </div>
        </div>
...
<Link style={{textDecoration : 'none', color : 'aliceblue', fontSize : '1.2rem', paddingLeft :'3rem',fontWeight : 'bold'}} href="/api/auth/logout">
 ...
 onClick={()=>{
  document.getElementsByClassName('sidebar-backdrop')[0].addEventListener('click',()=>{
    const sidebar = document.getElementsByClassName('sidebar-chat-history')[0].style.display = 'none'
  document.getElementsByClassName('sidebar-backdrop')[0].style.display = 'none'
  })
 }}></div>
